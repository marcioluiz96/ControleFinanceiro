...
public class CadastrarDispesa extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cadastrar_dispesa);
		...
		if (descricao.getText().toString().length() <= 0) {
			...
			try {
				...
				if (db.insert("despesas", "_id", ctv) > 0) {
					Toast.makeText(getBaseContext(), "Despesa cadastrada",
							Toast.LENGTH_SHORT).show();
					finish();
				} else {
					Toast.makeText(getBaseContext(),
							"ERRO!!! Despesa Não cadastrada",
							Toast.LENGTH_SHORT);
